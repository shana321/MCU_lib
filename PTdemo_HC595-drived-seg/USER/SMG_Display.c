...
void Put2Buff(SMG_CLASS* s,uint8_t c)
{
    uint8_t i=0;
    while(s->DisBuff[i] != '\0') {
        i++;
...
void SMG_gohome(SMG_CLASS* s)
{
    s->str_l = 0;
    s->SMG_L = 0;
}
void SMG_Clear(SMG_CLASS* s)
{
    uint8_t i=0;
...
SMG_CLASS* NewSMG(SMG_CLASS* s,uint8_t MAXLONG,uint8_t* disbuff,uint8_t MAX_buff,smg_cb mysmg)
{
